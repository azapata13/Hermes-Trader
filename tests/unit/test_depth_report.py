...
def test_depth_report_traces_each_resync(tmp_path, capsys):
    ...
    assert "initial subscription: reqId=10001" in out
    assert "resync #1: reqId=20001" in out
    assert "structural_violation" in out
    assert "UPDATE BID pos=9" in out
    assert "position_out_of_range" in out
    assert "trigger: structural_violation" in out
    assert "persistent_bbo_mismatch" in out
